 ...
 * @method order_MessageService getInstance()
 */
class order_MessageService extends f_persistentdocument_DocumentService
{
	...
	{
		return $this->getPersistentProvider()->createQuery('modules_order/message');
	}
		...
		foreach ($messages as $message)
		{
			if ($includeDetails)
			{
				...
				if ($message->isSentByCustomer())
				{
					...
					$messageInfo['label'] = LocaleService::getInstance()->trans('m.order.bo.general.message-sent-by-customer', array('ucf'));
				}
				else
				{
					...
					$messageInfo['label'] = LocaleService::getInstance()->trans('m.order.bo.general.message-sent-to-customer', array('ucf'), array('sender' => $messageInfo['senderFullName']));
				}

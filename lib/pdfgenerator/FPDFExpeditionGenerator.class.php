...
{	
	...
	{
		$this->AliasNbPages();
		$this->SetTitle(LocaleService::getInstance()->transFO('m.order.fo.packet-form', array('ucf', 'lab')) . ' ' . $this->expedition->getLabel(), true);
		$this->AddPage();
		$lineHeight = 3;
	...
	protected function generateShippingInfos()
	{
		...
		{
			$shippingCells[$ls->transFO('m.order.document.expedition.shippingdate')] = date_Formatter::toDefaultDateTime($this->expedition->getUIShippingDate());
		}
		if ($this->expedition->getTrackingNumber())
		{
			$shippingCells[$ls->transFO('m.order.frontoffice.tracking-number')] = $this->expedition->getTrackingNumber();
		}
		if ($this->expedition->getTrackingText())
		{
			$shippingCells[$ls->transFO('m.order.frontoffice.tracking-text')] = $this->expedition->getTrackingText();
		}
	...
	protected function generateBody()
	{
		$this->SetFontSize(13);
		$ls = LocaleService::getInstance();
		$title = $this->convertUTF8String($ls->transFO('m.order.frontoffice.expedition-lines', array('ucf')));
		...
		foreach ($packetByExpedition as $packetLines)
		{
			...
			{
				$packetInfos[] = $ls->transFO('m.order.frontoffice.packet', array('ucf', 'lab')) . ' ' . $packetLine->getPacketNumber();
			}
			if ($packetLine->getTrackingNumber())
			{
				$packetInfos[] = $ls->transFO('m.order.frontoffice.tracking-number', array('ucf', 'lab')) . ' ' . $packetLine->getTrackingNumber();
				if ($packetLine->getTrackingText())
				{
					$packetInfos[] =  $ls->transFO('m.order.frontoffice.tracking-text', array('ucf', 'lab')) . ' ' . $packetLine->getTrackingText();
				}
			}
			if ($packetLine->getStatus())
			{
				$packetInfos[] = $ls->transFO('m.order.frontoffice.status', array('ucf', 'lab')) . ' ' . $packetLine->getStatusLabel();
			}
			if ($packetLine->getReceiveDate())
			{
				$packetInfos[] = $ls->transFO('m.order.frontoffice.receive-date', array('ucf', 'lab')) . ' ' . date_Formatter::toDefaultDateTime($packetLine->getUIReceiveDate());
			}
			if ($packetLine->getDeliveryDate())
			{
				$packetInfos[] = $ls->transFO('m.order.frontoffice.delivery-date', array('ucf', 'lab')) . ' ' . date_Formatter::toDefaultDateTime($packetLine->getUIDeliveryDate());
			}
	...
	protected function generateExpeditionLineHeaders()
	{
		...
		$this->Cell($expeditionLineColumnSizes[0], 5, $this->convertUTF8String($ls->transFO('m.order.frontoffice.designation', array('ucf'))), 1, 0, 'C', true);
		$this->Cell($expeditionLineColumnSizes[1], 5, $this->convertUTF8String($ls->transFO('m.order.frontoffice.codereference', array('ucf'))), 1, 0, 'C', true);
		$this->Cell($expeditionLineColumnSizes[2], 5, $this->convertUTF8String($ls->transFO('m.order.frontoffice.quantity', array('ucf'))), 1, 0, 'C', true);
		$this->Ln();
	}

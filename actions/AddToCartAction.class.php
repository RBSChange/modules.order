	...
	public function _execute($context, $request)
	{
		...
		if ($cs->checkAddToCart($cart, $shop, $products, $quantities, true, $paramsToRedirect))
		{
			...
			if ($cs->addProductToCart($cart, $product, $quantity, $shop))
			{
				...
				$cart->addSuccessMessage(LocaleService::getInstance()->transFO('m.order.fo.product-label-added-link', array('ucf', 'html'), $replacements));
				$cart->refresh();
			}
		}
		
		// Redirect.
		HttpController::getInstance()->redirectToUrl($backUrl ? $backUrl : $cartUrl);
	}

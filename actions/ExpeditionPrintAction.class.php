 ...
 */
class order_ExpeditionPrintAction extends change_Action
{
	/**
	 * @return boolean
	 */
		...
		if (count($expeditions) == 1)
		{
			...
			if ($address instanceof  order_persistentdocument_shippingaddress)
			{
				...
				if ($page)
				{
					...
					$path = f_util_FileUtils::buildProjectPath('libs', 'qrcode', 'qrlib.php');
					include $path;
	...
	{
		$path = f_util_FileUtils::buildProjectPath('libs', 'barcode', 'php-barcode-2.0.1.php');	
		require_once($path);
		$width = 280;
		$barCodeHeight = 50;
